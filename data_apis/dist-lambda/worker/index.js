...
var lambda_worker_exports = {};
__export(lambda_worker_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(lambda_worker_exports);
...
function env(key, fallback) {
  ...
  if (v === void 0) throw new Error(`Missing env var: ${key}`);
  return v;
}
...
// src/infra/aws/dynamoJobRepository.ts
var import_client_dynamodb = require("@aws-sdk/client-dynamodb");
var import_lib_dynamodb = require("@aws-sdk/lib-dynamodb");
var DynamoJobRepository = class {
  doc;
  table;
  constructor(config2) {
    const client = new import_client_dynamodb.DynamoDBClient({
      region: config2.region,
      ...config2.dynamoEndpoint && { endpoint: config2.dynamoEndpoint }
    });
    this.doc = import_lib_dynamodb.DynamoDBDocumentClient.from(client, {
      marshallOptions: { removeUndefinedValues: true }
    });
    this.table = config2.ddbJobsTable;
  }
  async create(job) {
    await this.doc.send(
  ...
  async findById(jobId) {
    ...
    if (!res.Item) return void 0;
    return res.Item;
  }
  async claimJob(jobId, leaseUntil) {
    try {
      await this.doc.send(
        new import_lib_dynamodb.UpdateCommand({
          TableName: this.table,
          Key: { job_id: jobId },
          UpdateExpression: "SET #status = :running, lease_until = :lease, updated_at = :now",
      ...
      if (err instanceof import_client_dynamodb.ConditionalCheckFailedException) return false;
      throw err;
    }
  }
...
// src/infra/aws/dynamoStateStore.ts
var import_client_dynamodb2 = require("@aws-sdk/client-dynamodb");
var import_lib_dynamodb2 = require("@aws-sdk/lib-dynamodb");
var DynamoStateStore = class {
  doc;
  table;
  constructor(config2) {
    const client = new import_client_dynamodb2.DynamoDBClient({
      region: config2.region,
      ...config2.dynamoEndpoint && { endpoint: config2.dynamoEndpoint }
    });
    this.doc = import_lib_dynamodb2.DynamoDBDocumentClient.from(client, {
      marshallOptions: { removeUndefinedValues: true }
    });
    this.table = config2.ddbStateTable;
  }
...
// src/infra/aws/s3ConfigStore.ts
var import_client_s3 = require("@aws-sdk/client-s3");
var S3ConfigStore = class {
  s3;
  bucket;
  constructor(config2) {
    this.s3 = new import_client_s3.S3Client({
      region: config2.region,
      ...config2.s3Endpoint && {
        endpoint: config2.s3Endpoint,
        forcePathStyle: true
      }
    });
    this.bucket = config2.s3ConfigBucket;
  }
  async putConfig(connectionId, jobId, config2) {
...
// src/infra/aws/s3DataLakeWriter.ts
var import_client_s32 = require("@aws-sdk/client-s3");
var S3DataLakeWriter = class {
  s3;
  bucket;
  // Per-dataset state kept across writeChunk() calls until finalise()
  segmentKeys = /* @__PURE__ */ new Map();
  segmentCounters = /* @__PURE__ */ new Map();
  constructor(config2) {
    this.s3 = new import_client_s32.S3Client({
      region: config2.region,
      ...config2.s3Endpoint && {
        endpoint: config2.s3Endpoint,
        forcePathStyle: true
    ...
    this.bucket = config2.s3DatalakeBucket;
  }
  async writeChunk(events, datasetId) {
  ...
  async finalise(datasetId, metadata) {
...
// src/infra/connectors/esgCsvBatchConnector.ts
...
// src/infra/connectors/esgCsvBatchConnector.ts
var BATCH_SIZE = 5e3;
var EsgCsvBatchConnector = class {
  s3;
  constructor(config2) {
    this.s3 = new import_client_s33.S3Client({
      region: config2.region,
      ...config2.s3Endpoint && {
        endpoint: config2.s3Endpoint,
        forcePathStyle: true
  ...
  async fetchIncremental(sourceSpec, prevState, onBatch) {
    const objectKeys = await this.resolveObjectKeys(sourceSpec);
    const filteredKeys = prevState?.last_processed_object_key ? objectKeys.filter((k) => k > prevState.last_processed_object_key) : objectKeys;
  ...
  async streamBatches(bucket, key, objKey, delimiter, hasHeader, onBatch) {
    const res = await this.s3.send(
      new import_client_s33.GetObjectCommand({ Bucket: bucket, Key: key })
    );
    const stream = res.Body;
    const parser = parse({
      delimiter,
      columns: hasHeader ? true : (header) => header.map((_, i) => `col_${i}`),
...
// src/infra/connectors/connectorFactory.ts
var registry = {
  esg_csv_batch: (config2) => new EsgCsvBatchConnector(config2)
};
function createConnector(connectorType, config2) {
...
// src/application/normalizers/normalizeEsgMetrics.ts
function normalizeEsgMetrics(records, config2, runTimestamp) {
  return records.map((r) => {
    ...
    return {
      ...
      attribute: {
        ...
        metric_value: row.metric_value && row.metric_value !== "null" ? Number(row.metric_value) : null,
        metric_year: year,
        ...
        nb_points_of_observations: row.nb_points_of_observations && row.nb_points_of_observations !== "null" ? Number(row.nb_points_of_observations) : null,
...
// src/application/normalizers/normalizeHousingSales.ts
...
var normalizeHousingSales = function normalizeHousingSales2(records, config2, runTimestamp) {
  const results = [];
...
// src/application/normalizers/index.ts
var registry2 = {
  esg_v1: normalizeEsgMetrics,
  housing_v1: normalizeHousingSales
};
function getNormalizer(mappingProfile) {
  const fn = registry2[mappingProfile];
  if (!fn) {
...
// src/application/worker/runJob.ts
var LEASE_DURATION_MS = 10 * 60 * 1e3;
async function runJob(jobId, deps2) {
  const leaseUntil = new Date(Date.now() + LEASE_DURATION_MS).toISOString();
  const claimed = await deps2.jobRepo.claimJob(jobId, leaseUntil);
  if (!claimed) return;
  try {
    const job = await deps2.jobRepo.findById(jobId);
    if (!job) throw new Error(`job record disappeared: ${jobId}`);
    const config2 = await deps2.configStore.getConfig(job.config_ref);
    const prevState = config2.ingestion_mode === "incremental" ? await deps2.stateStore.getState(config2.connection_id) : void 0;
    const connector = deps2.connectorFactory(config2.connector_type);
    const datasetId = v4_default();
    const runTimestamp = (/* @__PURE__ */ new Date()).toISOString();
    const normalize = getNormalizer(config2.mapping_profile);
    let totalEvents = 0;
    const newState = await connector.fetchIncremental(
      config2.source_spec,
      prevState,
      async (batch) => {
        const events = normalize(batch, config2, runTimestamp);
        await deps2.dataLakeWriter.writeChunk(events, datasetId);
        totalEvents += events.length;
      }
    );
    const datasetUri = await deps2.dataLakeWriter.finalise(datasetId, {
      data_source: config2.data_source,
      dataset_type: config2.dataset_type,
      time_object: { timestamp: runTimestamp, timezone: config2.timezone },
      total_events: totalEvents
    });
    if (config2.ingestion_mode === "incremental") {
      await deps2.stateStore.saveState({
        connection_id: config2.connection_id,
        ...newState,
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
      });
    }
    await deps2.jobRepo.updateStatus(jobId, "DONE", { dataset_id: datasetUri });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    await deps2.jobRepo.updateStatus(jobId, "FAILED", { error: message });
    throw err;
  }
}
